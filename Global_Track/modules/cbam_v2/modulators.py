...
class Cls_Modulator(nn.Module):
    def __init__(self,
        ...
        self.roi_extractor = SingleRoIExtractor(
            ...
            out_channels=channels,
            featmap_strides=strides,
            finest_scale=56)     # for atss , p3-p6   56
        ...
        self.proj_out = nn.ModuleList([
            ...
            for _ in range(featmap_num)])
        # by lc attention
        self.channel_weight = CBAM(gate_channels=256, no_spatial=True)
    ...
    def inference(self, feats_x, query_feats, modulator, track_flag,joint_flag,switch_up ):
        ...
        for i in range(n_imgs):
            ...
            for j in range(n_instances):
                query = query_feats[i][j:j + 1]  # query应该是1,256,7,7
                gallary = [f[i:i + 1] for f in feats_x]
                embedings = self.generate_embeddings(query)  # 1,256,1,1 所有层是共用的
                ...
                '''  version 3  '''
                # if switch_up:
                #     attention_map_list = [self.xcorr_fast(gallary[k], self.proj_em[k](embedings_))
                #                           # gallary的每个元素都是其中一个特征层，faets_x本来就是5元素的list，每个元素是batch大小，batch为图像个数，i作指示
                #                           for k in range(len(gallary))]
                #     # out_ij = [p * o for p, o in zip(gallary, attention_map_list)]
                #     m = nn.Sigmoid()
                #     attention_map_list_new = [m(attention_map_list[k]) for k in range(len(attention_map_list))]
                #     out_ij = [p * o for p, o in zip(gallary, attention_map_list_new)]
                # else:
                #     out_ij = gallary
                #
                # m = torch.nn.Softmax(dim=2)
                # attention_map_list_new = [m(attention_map_list[k].view(1,1,-1)) for k in range(len(attention_map_list))]
                # shapes_list = [attention_map_list[k].shape[2:] for k in range(len(attention_map_list_new))]
                # attention_map_list_new = [attention_map_list_new[k].view(1,1,shapes_list[k][0],shapes_list[k][1]) for k in range(len(attention_map_list_new))]
                # out_ij = [p * o for p, o in zip(gallary, attention_map_list_new)]

                '''  version channel weight 11.1 '''
                # if switch_up:
                #     attention_map_list = [self.xcorr_fast(gallary[k], self.proj_em[k](embedings_))
                #                           # gallary的每个元素都是其中一个特征层，faets_x本来就是5元素的list，每个元素是batch大小，batch为图像个数，i作指示
                #                           for k in range(len(gallary))]
                ...
                #     # import numpy as np
                #     # import cv2
                #     # heatmap = attention_map_list_new[0][0, ...].cpu().permute(1, 2, 0).numpy() * 255
                ...
                ''' version channel weight 10.28'''
                # if switch_up:
                #     corr_emd = query  # (7,7)
                #     attention_list = [self.xcorr_depthwise_1(gallary[k], self.proj_modulator_cls[k](corr_emd))
                #                       # gallary的每个元素都是其中一个特征层，feats_x本来就是5元素的list，每个元素是batch大小，batch为图像个数，i作指示
                #                       for k in range(len(gallary))]
                #     attention_list = [p(o) for p, o in zip(self.proj_out_cls, attention_list)]  # 原来没有 10.26
                #     scale = []
                #     for att in attention_list:
                #         avg_pool = F.avg_pool2d(att, (att.size(2), att.size(3)), stride=(att.size(2),att.size(3)))
                #         max_pool = F.max_pool2d(att, (att.size(2), att.size(3)), stride=(att.size(2), att.size(3)))
                #         channel_att_avg = self.mlp(avg_pool)
                #         channel_att_max = self.mlp(max_pool)
                #         channel_att_sum = channel_att_avg + channel_att_max
                #         scale.append(F.sigmoid(channel_att_sum).unsqueeze(2).unsqueeze(3).expand_as(att))
                #     out_ij = [p * o for p, o in zip(gallary, scale)]
                # else:
                #     out_ij = gallary
                ''' version channel weight 10.26-27'''
                # if switch_up:
                #     corr_emd = query  # (3, 3)
                #     for p in self.proj_embed:
                #         corr_emd = p(corr_emd)
                #     embedings_ = nn.functional.adaptive_avg_pool2d(corr_emd, (1, 1)).view(1, -1)
                #     # corr_emd = nn.functional.adaptive_avg_pool2d(query, (3, 3))   # 3*3
                #     attention_list = [self.xcorr_depthwise(gallary[k], self.proj_modulator_cls[k](corr_emd))
                #                       # gallary的每个元素都是其中一个特征层，feats_x本来就是5元素的list，每个元素是batch大小，batch为图像个数，i作指示
                #                       for k in range(len(gallary))]
                #     # attention_list = [self.xcorr_depthwise(gallary[k], corr_emd)
                #     #           # gallary的每个元素都是其中一个特征层，feats_x本来就是5元素的list，每个元素是batch大小，batch为图像个数，i作指示
                #     #           for k in range(len(gallary))]
                #     attention_list = [p(o) for p, o in zip(self.proj_3, attention_list)]  # 原来没有 10.26
                #     scale = []
                #     k = 0
                #     for att in attention_list:
                #         avg_pool = F.avg_pool2d(att, (att.size(2), att.size(3)), stride=(att.size(2),att.size(3)))
                #         max_pool = F.max_pool2d(att, (att.size(2), att.size(3)), stride=(att.size(2), att.size(3)))
                #         # channel_att_avg = self.mlp[k](avg_pool)
                #         # channel_att_max = self.mlp[k](max_pool)
                #         channel_att_avg = self.mlp(avg_pool)
                #         channel_att_max = self.mlp(max_pool)
                #         channel_att_sum = channel_att_avg + channel_att_max
                #         scale.append(F.sigmoid(channel_att_sum).unsqueeze(2).unsqueeze(3).expand_as(att))
                ...
                ''' version channel weight 10_16_17'''
                # if switch_up:
                #     corr_emd = nn.functional.adaptive_avg_pool2d(query, (3, 3))   # 3*3
                #     # attention_list = [self.xcorr_depthwise(gallary[k], self.proj_modulator_cls[k](corr_emd))
                #     #                   # gallary的每个元素都是其中一个特征层，feats_x本来就是5元素的list，每个元素是batch大小，batch为图像个数，i作指示
                ...
                if joint_flag:
                    ...
                    # yield embedings_, out_ij_track, out_ij_shot, i, j

                # yield embedings_, out_ij, i, j   # for special reason, comment
                yield att, out_ij, i, j   # for special reason, comment
