...
def _write_voc_results_files(json_dataset, all_boxes, salt):
    filenames = []
    image_set_path = voc_info(json_dataset)['image_set_path']  # './data/fsod/new_val.txt'
    ...
    for cls_ind, cls in enumerate(json_dataset.classes): ############## need to change json_dataset.classes
        ...
        filename = _get_voc_results_file_template(json_dataset,
                                                  salt).format(cls)    # just a output_dir dir, by lc, totally 3000 txts
...
def _do_python_eval(json_dataset, salt, output_dir='output'):
    ...
    cachedir = os.path.join('data/VOC' + year, 'annotations_cache')  # 'data/VOC_tes/annotations_cache'
    aps_50 = []
    cls_ls = []
    # The PASCAL VOC metric changed in 2010
    # use_07_metric = True if int(year) < 2010 else False  # by lc
    use_07_metric = True
    logger.info('VOC07 metric? ' + ('Yes' if use_07_metric else 'No'))
    # if not os.path.isdir('/media/lc/61854679-da08-4161-9df3-62beb989649f/lc/GlobalTrackATSS/tests/' + output_dir):
    #     os.mkdir(output_dir)
    ...
    # load annots
    recs = {}
    with open('/home/lc/Downloads/database/fsod/annotations/fsod_test.json', 'r') as f:
    ...
    for _, cls in enumerate(json_dataset.classes):
        ...
        rec, prec, ap = voc_eval(
            filename, anno_path, recs, imagenames, cls, cachedir, ovthresh=0.15,  # 0.5
