...
def run_vot_exp(base_tracker, base_param, ref_tracker, ref_param, use_new_box, prj_dir,vis=False):
    torch.set_num_threads(1)
    dir_name = base_tracker + '_' + base_param + '___' + ref_tracker + '_' + ref_param
    save_root = os.path.join('/media/lc/61854679-da08-4161-9df3-62beb989649f/lc/Desktop/LT-cooperation/vis', dir_name)
    ...
    # init_box = [360.99,325.11,25.714,111.76]
    # imagefile = '/media/lc/da5a7293-2b4e-42c2-9034-7e975b6e61cc/database/vot2018_LT/ballet/color/00000001.jpg'
    if not imagefile:
        sys.exit(0)
    if vis:
        '''for vis'''
    ...
    while True:
        imagefile = handle.frame()
        # imagefile = '/media/lc/da5a7293-2b4e-42c2-9034-7e975b6e61cc/database/vot2018_LT/ballet/color/00000002.jpg'
        if not imagefile:
            break
        image = cv2.cvtColor(cv2.imread(imagefile), cv2.COLOR_BGR2RGB)  # Right
